...
for i = 1: size(A, 1)
    mumAlleles = genotypesToAlleles(A(i, 2),:);
    dadAlleles = genotypesToAlleles(A(i, 3),:);
    childAlleles = genotypesToAlleles(A(i, 1), :);
    for m = 1:2
        for n = 1:2
            parent = sort([mumAlleles(m), dadAlleles(n)]);
            if isequal(parent, childAlleles)
